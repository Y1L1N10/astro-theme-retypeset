---
title: Agent Skills 完整配置指南
published: 2026-01-22
updated: 2026-01-22
description: 深入解析 Claude Agent Skills 系统架构、配置方法和最佳实践，涵盖渐进式加载机制、文件结构规范及四种部署方式的完整指南
tags: [AI, Claude, Agent Skills, 开发工具, 最佳实践]
lang: ''
toc: true
draft: false
pin: 0
abbrlink: agent-skills-guide
---
...
export async function exampleHandler(req: Request, res: Response, next: NextFunction) {
  try {
    // 1. 验证输入
...
对于复杂的 Skill，建议使用多文件结构，将 `SKILL.md` 作为导航中心，其他文件提供详细内容。这种方式可以优化加载性能，避免一次性加载过多内容。
...
**作用**：提供概览和导航，引导 Claude Code 在需要时加载其他文件
...
5. **在 SKILL.md 中提供导航**
   ...
   - `SKILL.md` (本文件) - 快速开始和导航
   - `reference.md` - 完整 API 参考文档
   - `examples.md` - 实用示例集合
   - `advanced.md` - 高级功能和配置
   - `scripts/` - 可执行工具脚本
   ```
...
api-testing/
├── SKILL.md              # 概览和快速开始
├── reference.md          # API 参数详细说明
├── examples.md           # 各种测试场景示例
├── advanced.md           # 性能测试、安全测试
├── troubleshooting.md    # 常见问题解决
├── scripts/
│   ├── test_endpoint.py  # 单端点测试
│   ├── batch_test.py     # 批量测试
│   ├── load_test.py      # 负载测试
│   └── utils/
│       ├── validators.py # 响应验证工具
│       └── reporters.py  # 测试报告生成
